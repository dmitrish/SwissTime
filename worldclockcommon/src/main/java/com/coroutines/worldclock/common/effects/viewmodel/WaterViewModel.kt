package com.coroutines.worldclock.common.effects.viewmodel

import android.util.Log
import androidx.compose.ui.geometry.Offset
import androidx.lifecycle.ViewModel
import com.coroutines.worldclock.common.effects.model.Wave
import com.coroutines.worldclock.common.effects.model.WaveShaderParams
 ...
 * Handles wave lifecycle, creation with cooldown, cleanup, and provides uniform data formatted for
 * the shader.
 */
class WaterViewModel : ViewModel() {
  ...
  /**
   * Removes waves whose amplitude after damping falls below the CPU threshold. Should be called
   * regularly with current system time.
   *
   * @param currentTimeSeconds current system time in seconds
   */
  fun cleanupWaves(currentTimeSeconds: Float) {
    _waves.value =
      _waves.value.filter { wave ->
  ...
  /**
   * Adds a new wave at [position] if cooldown period for [pointerId] has elapsed. If max waves
   * limit is reached, removes the weakest wave first.
   *
  ...
  fun addWave(position: Offset, pointerId: Int, currentTimeSeconds: Float) {
    ...
    if (
      currentTimeSeconds - lastEmission >= emissionCooldownSeconds &&
      ...
      Log.d(
        "WaveDebug",
        "Wave added at (${position.x}, ${position.y}) at time $currentTimeSeconds. Total waves: ${currentWaves.size}"
      )
    }
  }
