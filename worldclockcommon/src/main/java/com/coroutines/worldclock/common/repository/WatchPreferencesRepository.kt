...
private val Context.dataStore: DataStore<Preferences> by
  preferencesDataStore(name = "watch_preferences")
...
class WatchPreferencesRepository(private val context: Context) {
  ...
  companion object {
    ...
    private val USE_US_TIME_FORMAT_KEY =
      androidx.datastore.preferences.core.booleanPreferencesKey("use_us_time_format")
    private val USE_DOUBLE_TAP_FOR_REMOVAL_KEY =
      androidx.datastore.preferences.core.booleanPreferencesKey("use_double_tap_for_removal")
  ...
  val selectedWatchName: Flow<String?> =
    context.dataStore.data.map { preferences -> preferences[SELECTED_WATCH_KEY] }

  // Get the currently selected timezone ID
  val selectedTimeZoneId: Flow<String?> =
    context.dataStore.data.map { preferences -> preferences[SELECTED_TIMEZONE_KEY] }

  // Get the time format preference (true for US format, false for International format)
  val useUsTimeFormat: Flow<Boolean> =
  ...
  val useDoubleTapForRemoval: Flow<Boolean> =
    context.dataStore.data.map { preferences ->
  ...
  suspend fun saveSelectedWatch(watchName: String) {
    context.dataStore.edit { preferences -> preferences[SELECTED_WATCH_KEY] = watchName }
  }

  // Save the selected timezone ID
  suspend fun saveSelectedTimeZone(timeZoneId: String) {
    context.dataStore.edit { preferences -> preferences[SELECTED_TIMEZONE_KEY] = timeZoneId }
  }

  // Save the time format preference
  suspend fun saveTimeFormat(useUsFormat: Boolean) {
    context.dataStore.edit { preferences -> preferences[USE_US_TIME_FORMAT_KEY] = useUsFormat }
  }
  ...
  fun getWatchTimeZoneId(watchName: String, scope: CoroutineScope): SharedFlow<String?> =
    context.dataStore.data
      .map { preferences ->
        val key = stringPreferencesKey("${WATCH_TIMEZONE_PREFIX}${watchName}")
        preferences[key]
      }
      .shareIn(scope = scope, started = SharingStarted.WhileSubscribed(5000), replay = 0)

  // New: Flow without shareIn, for per-watch StateFlow building
  fun watchTimeZoneIdFlow(watchName: String): Flow<String?> =
    context.dataStore.data.map { preferences ->
      val key = stringPreferencesKey("${WATCH_TIMEZONE_PREFIX}${watchName}")
      preferences[key]
    }
  ...
  suspend fun clearSelectedWatch() {
    context.dataStore.edit { preferences -> preferences.remove(SELECTED_WATCH_KEY) }
  }

  // Clear the selected timezone
  suspend fun clearSelectedTimeZone() {
    context.dataStore.edit { preferences -> preferences.remove(SELECTED_TIMEZONE_KEY) }
  }
  ...
  val selectedWatchNames: Flow<Set<String>> =
    context.dataStore.data.map { preferences -> preferences[SELECTED_WATCHES_KEY] ?: emptySet() }

  // Save the list of selected watch names
  suspend fun saveSelectedWatches(watchNames: Set<String>) {
    context.dataStore.edit { preferences -> preferences[SELECTED_WATCHES_KEY] = watchNames }
  }
  ...
  fun getSelectedWatchNamesBlocking(): Set<String> = runBlocking {
    context.dataStore.data
      .map { preferences -> preferences[SELECTED_WATCHES_KEY] ?: emptySet() }
      .first()
  }
  ...
  suspend fun clearAllSelectedWatches() {
    context.dataStore.edit { preferences -> preferences.remove(SELECTED_WATCHES_KEY) }
  }
}
