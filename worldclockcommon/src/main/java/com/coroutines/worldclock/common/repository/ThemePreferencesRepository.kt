...
class ThemePreferencesRepository(private val context: Context) {
  ...
  val darkMode: Flow<Boolean> =
    context.themeDataStore.data.map { preferences -> preferences[DARK_MODE_KEY] ?: false }

  // Save the theme mode
  suspend fun saveThemeMode(themeMode: ThemeMode) {
    context.themeDataStore.edit { preferences -> preferences[THEME_MODE_KEY] = themeMode.name }
  }

  // Save the dark mode preference
  suspend fun saveDarkMode(darkMode: Boolean) {
    context.themeDataStore.edit { preferences -> preferences[DARK_MODE_KEY] = darkMode }
  }
