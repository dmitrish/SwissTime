...
class RomaMarinaClock(private val context: Context, private val handler: Handler) : BaseClock {
  ...
  private fun drawClockFace(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    ...
    for (i in 0 until gridSize) {
      for (j in 0 until gridSize) {
        ...
        val distanceFromCenter =
          sqrt((x + squareSize / 2 - centerX).pow(2) + (y + squareSize / 2 - centerY).pow(2))
    ...
    canvas.drawText("ROMA-MARINA", centerX, centerY - radius * 0.3f, brandTextPaint)

    // Draw model text
    modelTextPaint.textSize = radius * 0.07f
    canvas.drawText("MILITARE", centerX, centerY - radius * 0.15f, modelTextPaint)
  }
  ...
  private fun drawHourMarkersAndNumbers(
    ...
    for (i in 0 until 12) {
      ...
      canvas.rotate(i * 30f, markerX, markerY)
  ...
  private fun drawDateWindow(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    ...
    canvas.drawText(date.toString(), dateX, dateY + radius * 0.03f, datePaint)
  }
