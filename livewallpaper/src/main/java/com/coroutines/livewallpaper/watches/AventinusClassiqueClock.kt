...
class AventinusClassiqueClock(private val context: Context, private val handler: Handler) :
  BaseClock {
  ...
  private fun drawGuillochePattern(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    ...
    for (i in 1..circleCount) {
      canvas.drawCircle(centerX, centerY, guillocheRadius - (i * circleSpacing), guillochePaint)
    }
  ...
  private fun drawLogo(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    // Draw "AVENTINUS" text
    canvas.drawText("AVENTINUS", centerX, centerY - radius * 0.3f, logoPaint)

    // Draw signature (Aventinus watches often have a secret signature)
    canvas.drawText("No. 1947", centerX, centerY + radius * 0.4f, signaturePaint)
  }
}
