...
/** A class that handles rendering the Pontifex Chronometra watch face. */
class PontifexChronometraClock(private val context: Context, private val handler: Handler) :
  BaseClock {
  ...
  private fun drawHourMarkers(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    for (i in 0 until 12) {
      ...
      if (i % 3 == 0) {
        ...
        val angleToCenter =
          Math.atan2((centerY - markerY).toDouble(), (centerX - markerX).toDouble()).toFloat()
        ...
        // Draw the teardrop
        path.quadTo(controlX1, controlY1, markerX, markerY)
  ...
  private fun drawDateWindow(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    ...
    canvas.drawText(date.toString(), dateWindowX, dateWindowY + radius * 0.02f, datePaint)
  }
  ...
  private fun drawClockHands(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    ...
    hourHandPath.lineTo(
      centerX +
        sin(hourAngle + Math.PI / 2).toFloat() * radius * 0.04f +
        sin(hourAngle).toFloat() * radius * 0.2f,
      centerY -
        cos(hourAngle + Math.PI / 2).toFloat() * radius * 0.04f -
        cos(hourAngle).toFloat() * radius * 0.2f
    ) // Right shoulder
    hourHandPath.lineTo(
    ...
    hourHandPath.lineTo(
      centerX - sin(hourAngle + Math.PI / 2).toFloat() * radius * 0.04f +
        sin(hourAngle).toFloat() * radius * 0.2f,
      centerY + cos(hourAngle + Math.PI / 2).toFloat() * radius * 0.04f -
        cos(hourAngle).toFloat() * radius * 0.2f
    ) // Left shoulder
    hourHandPath.close()
    ...
    minuteHandPath.lineTo(
      centerX +
        sin(minuteAngle + Math.PI / 2).toFloat() * radius * 0.03f +
        sin(minuteAngle).toFloat() * radius * 0.3f,
      centerY -
        cos(minuteAngle + Math.PI / 2).toFloat() * radius * 0.03f -
        cos(minuteAngle).toFloat() * radius * 0.3f
    ) // Right shoulder
    minuteHandPath.lineTo(
    ...
    minuteHandPath.lineTo(
      centerX - sin(minuteAngle + Math.PI / 2).toFloat() * radius * 0.03f +
        sin(minuteAngle).toFloat() * radius * 0.3f,
      centerY + cos(minuteAngle + Math.PI / 2).toFloat() * radius * 0.03f -
        cos(minuteAngle).toFloat() * radius * 0.3f
    ) // Left shoulder
    ...
    canvas.save()
    canvas.rotate((secondAngle * 180 / Math.PI).toFloat(), ovalCenterX, ovalCenterY)
  ...
  private fun drawLogo(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    ...
    canvas.drawText("PONTIFEX", centerX, centerY - radius * 0.25f, brandTextPaint)

    // Draw "CHRONOMETRA" text
    brandTextPaint.textSize = radius * 0.05f
    canvas.drawText("CHRONOMETRA", centerX, centerY - radius * 0.15f, brandTextPaint)

    // Draw "SWISS MADE" text
    brandTextPaint.textSize = radius * 0.04f
    canvas.drawText("SWISS MADE", centerX, centerY + radius * 0.5f, brandTextPaint)
  }
}
