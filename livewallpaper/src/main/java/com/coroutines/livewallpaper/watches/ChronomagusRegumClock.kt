...
class ChronomagusRegumClock(private val context: Context, private val handler: Handler) :
  BaseClock {
  ...
  private fun drawLogo(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    ...
    canvas.drawText("CHRONOMAGUS", centerX, centerY - radius * 0.4f, logoPaint)

    // Draw "REGIUM" text
    canvas.drawText("REGIUM", centerX, centerY - radius * 0.2f, modelPaint)

    // Draw "Fabricatum Romae" text
    canvas.drawText("Fabricatum Romae", centerX, centerY + radius * 0.5f, swissMadePaint)

    // Draw "ULTRA-THIN" text
    canvas.drawText("ULTRA-THIN", centerX, centerY + radius * 0.3f, ultraThinPaint)
  }
}
