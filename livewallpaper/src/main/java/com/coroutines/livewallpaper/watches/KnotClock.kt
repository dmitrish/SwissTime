...
class KnotClock(private val context: Context, private val handler: Handler) : BaseClock {
  ...
  private val urushiBlackColor =
    Color.parseColor("#000000") // Deep jet black for the Urushi lacquer dial
  ...
  private val markersColor =
    Color.parseColor("#F5F5F5") // Bright silver hour markers for better contrast
  ...
  private val markerHighlightPaint =
    Paint().apply {
      color = Color.WHITE
      isAntiAlias = true
      style = Paint.Style.STROKE
  ...
  private fun drawHourMarkers(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    ...
    for (i in 0 until 12) {
      ...
      canvas.rotate((angle * 180 / Math.PI).toFloat() + 90, markerX, markerY)
  ...
  private fun drawClockHands(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    ...
    // Counterbalance
    canvas.drawCircle(centerX, centerY + radius * 0.15f, radius * 0.03f, secondHandFillPaint)
  ...
  private fun drawLogo(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
    // Draw "KNOT" text
    canvas.drawText("KNOT", centerX, centerY - radius * 0.3f, logoPaint)

    // Draw "URUSHI" text
    canvas.drawText("URUSHI", centerX, centerY + radius * 0.4f, subtitlePaint)
  }
