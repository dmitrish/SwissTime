...
/**
 * A horizontal pager for use in WallpaperService that supports multiple pages
 * with smooth animations and touch gestures.
 */
class WallpaperHorizontalPager(
    ...
    private val gestureDetector = GestureDetector(context, object : GestureDetector.SimpleOnGestureListener() {
        ...
        override fun onScroll(
            ...
            val newOffset = when {
                currentPage == 0 && dragOffset < 0 -> max(0f, dragOffset * 0.3f) // Resistance when at first page
                currentPage == pageCount - 1 && dragOffset > 0 -> min(0f, dragOffset * 0.3f) // Resistance when at last page
                else -> dragOffset
            }
    ...
    /**
     * Set a callback to be invoked when the page offset changes.
     * This should trigger a redraw of the wallpaper.
     */
    ...
    fun draw(canvas: Canvas, drawPage: (Canvas, Int, Float) -> Unit) {
        ...
        for (page in visiblePages) {
            ...
            val pageTransitionOffset = when (page) {
