...
@Composable
fun KnotUrushi(modifier: Modifier = Modifier, timeZone: TimeZone = TimeZone.getDefault()) {
  WatchFaceScaffold(
    modifier = modifier,
    timeZone = timeZone,
    ...
    animatedContent = { center, radius, currentTime, _ ->
      ...
      drawCircle(color = SilverHandsColor, radius = radius * 0.02f, center = center)
    }
  )
}
...
private fun DrawScope.drawKnotClockFace(center: Offset, radius: Float) {
  ...
  drawCircle(color = UrushiBlackColor, radius = radius * 0.95f, center = center)
}
...
private fun DrawScope.drawKnotUrushiTexture(center: Offset, radius: Float) {
  ...
  for (i in 0 until particleCount) {
    ...
    drawCircle(color = goldShade, radius = particleSize, center = Offset(x, y))
  }

  drawCircle(
    brush =
      Brush.radialGradient(
        colors = listOf(Color(0xFF050505), UrushiBlackColor),
        center = center,
        radius = radius * 0.95f
      ),
    radius = radius * 0.95f,
...
private fun DrawScope.drawKnotLogo(center: Offset, radius: Float) {
  ...
  drawContext.canvas.nativeCanvas.drawText("KNOT", center.x, center.y - radius * 0.3f, logoPaint)
