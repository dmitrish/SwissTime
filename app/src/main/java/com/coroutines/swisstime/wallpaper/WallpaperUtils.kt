...
fun WallPaperWatchesHorizontalPager(modifier: Modifier = Modifier) {
  ...
  Column(
    ...
    HorizontalPager(
      state = pagerState,
      modifier = Modifier.fillMaxSize().aspectRatio(1f) // Square aspect ratio for watches
    ) { page ->
      ...
      Box(
        modifier =
          Modifier.fillMaxSize()
            ...
            clickable { launchDigitalClockWallpaperPicker(context, name) },
  ...
  DisposableEffect(Unit) { onDispose { watches.forEach { it.second.destroy() } } }
}
...
private fun WatchCanvas(watch: BaseClock) {
  Canvas(modifier = Modifier.fillMaxSize().padding(16.dp)) {
    ...
    drawIntoCanvas { canvas -> watch.draw(canvas.nativeCanvas) }
  }
}
/**
 * A composable that displays a card with information about the digital clock wallpaper and a button
 * to set it as the device wallpaper.
 */
@Composable
fun DigitalClockWallpaperCard() {
  ...
  Card(
    ...
    Column(
      ...
      Text(
        text =
          "Display the current time on your home screen with this simple digital clock live wallpaper.",
        style = MaterialTheme.typography.bodyMedium,
        textAlign = TextAlign.Center
      )
      ...
      Button(onClick = {}) { Text("Set as Wallpaper") }
    }
  }
}
