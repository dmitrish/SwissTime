...
fun SwissTimePager(
  ...
  BoxWithConstraints(modifier = Modifier.fillMaxSize()) {
    ...
    val sidePadding = ((viewportWidth - pageWidth) / 2f).coerceAtLeast(0.dp)

    val customSnapAnimationSpec = tween<Float>(durationMillis = 300, easing = EaseOutCubic)

    val customFlingBehavior =
      PagerDefaults.flingBehavior(state = pagerState, snapAnimationSpec = customSnapAnimationSpec)
    ...
    HorizontalPager(
      ...
      Box(
        modifier = Modifier.fillMaxSize().zIndex(1f - pageOffset.coerceIn(0f, 1f)),
        ...
        var mod = Modifier.size(watchSize).then(clickableModifier)
        ...
        Box(modifier = mod, contentAlignment = Alignment.Center) { pageContent(page) }
      }
    }
  }
}
