...
fun WallpaperSelectionCard(
  ...
  val selectedName: String? by
    wallpaperPreferenceRepository.selectedWallpaperName.collectAsState(initial = null)

  // Check if the currently applied wallpaper is from this app (live wallpaper)
  val wm = WallpaperManager.getInstance(context)
  val activeFromThisApp: Boolean =
    try {
      val info: WallpaperInfo? =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) wm.wallpaperInfo else wm.wallpaperInfo
  ...
  Card(
    ...
    colors = CardDefaults.cardColors(containerColor = DarkNavy.darken(0.3f))
  ) {
      ...
      if (activeFromThisApp && selectedWatch != null) {
        ...
        Button(
          onClick = {
            ...
            scope.launch { wallpaperPreferenceRepository.saveSelectedWallpaperName("") }
          },
