...
class ThemeViewModel(private val themePreferencesRepository: ThemePreferencesRepository) :
  ViewModel() {

  // Get the theme mode as a StateFlow
  val themeMode: StateFlow<ThemeMode> =
    themePreferencesRepository.themeMode.stateIn(
  ...
  val darkMode: StateFlow<Boolean> =
    themePreferencesRepository.darkMode.stateIn(
      scope = viewModelScope,
      started = SharingStarted.WhileSubscribed(5000),
      initialValue = false
    )
  ...
  fun saveThemeMode(themeMode: ThemeMode) {
    viewModelScope.launch { themePreferencesRepository.saveThemeMode(themeMode) }
  }

  // Save the dark mode preference
  fun saveDarkMode(darkMode: Boolean) {
    viewModelScope.launch { themePreferencesRepository.saveDarkMode(darkMode) }
  }

  // Reset to default theme preferences
  fun resetThemePreferences() {
    viewModelScope.launch { themePreferencesRepository.resetThemePreferences() }
  }

  /** Factory for creating ThemeViewModel instances */
  class Factory(private val themePreferencesRepository: ThemePreferencesRepository) :
