 ...
 *
 * Note: This test will not work as-is because AppVersionSection depends on Android-specific
 * components like Context and PackageManager, which are not available in the JVM environment where
 * Paparazzi tests run.
 *
 * In a real-world scenario, there are several approaches to handle this:
 * 1. Run the tests on an Android device or emulator using screenshot testing libraries like Shot
 ...
 *
 * For this example, we're showing the structure of the test, but it won't actually run successfully
 * due to the Android dependencies.
 */
class AppVersionSectionPaparazziTest {
  ...
  fun setup() {
    ...
    mockPackageInfo =
      mockk<PackageInfo>(relaxed = true).apply {
        versionName = testVersionName
        versionCode = testVersionCode
      }

    mockPackageManager =
      mockk<PackageManager>(relaxed = true).apply {
    ...
    every { Toast.makeText(any<Context>(), any<String>(), any<Int>()) } returns mockToast
  }
      ...
      Surface(color = Color(0xFFFAFAFA)) {
        ThemedContent(darkTheme = false) {
          ...
          CompositionLocalProvider(LocalContext provides mockContext) {
            AppVersionSection(modifier = Modifier)
          }
        }
      }
    }
