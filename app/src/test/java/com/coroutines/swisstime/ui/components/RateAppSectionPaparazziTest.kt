 ...
 * from the app, with mockK to mock Android dependencies.
 *
 * The approach:
...
class RateAppSectionPaparazziTest {
  ...
  fun setup() {
    ...
    mockContext = mockk<Context>().apply { every { packageName } returns testPackageName }
      ...
      Surface(color = Color(0xFFFAFAFA)) {
        ThemedContent(darkTheme = false) {
          ...
          CompositionLocalProvider(LocalContext provides mockContext) {
            RateAppSection(modifier = Modifier)
          }
        }
      }
    }
