 ...
 *
 * This demonstrates how to use Paparazzi for screenshot testing without the complexity of mocking
 ...
 *
 * This approach follows the testing pyramid principle: use the simplest possible implementation
 * that meets the testing goals.
 */
class SettingsScreenPaparazziTest {
  ...
  /**
   * A simplified Settings Screen for testing that mimics the visual appearance of the real
   * SettingsScreen without its dependencies.
   *
   * This implementation:
   ...
   *
   * The goal is to test the visual appearance in different states (dark mode, different time
   * formats, etc.) without the complexity of the real implementation.
   */
  @Composable
  fun SimpleSettingsScreen(
    ...
    Surface(
      ...
      Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        ...
        // Appearance Card
        SettingsCard(title = "Appearance", darkMode = darkMode) {
          // Dark Mode Toggle
          SettingsToggle(
            title = "Dark Mode",
        ...
        // Time Format Card
        SettingsCard(title = "Time Format", darkMode = darkMode) {
          // Time Format Toggle
          SettingsToggle(
        ...
        SettingsCard(title = "Watch Removal Gesture", darkMode = darkMode) {
  ...
  @Composable
  fun SettingsCard(title: String, darkMode: Boolean, content: @Composable () -> Unit) {
    Card(
      modifier = Modifier.fillMaxWidth(),
      colors =
        CardDefaults.cardColors(containerColor = if (darkMode) Color(0xFF2D2D2D) else Color.White)
    ) {
  ...
  fun testSettingsScreen_lightTheme() {
    paparazzi.snapshot {
      SimpleSettingsScreen(darkMode = false, useUsTimeFormat = true, useDoubleTapForRemoval = false)
    }
  }
  ...
  fun testSettingsScreen_darkTheme() {
    paparazzi.snapshot {
      SimpleSettingsScreen(darkMode = true, useUsTimeFormat = true, useDoubleTapForRemoval = false)
    }
  }
  ...
  fun testSettingsScreen_doubleTapRemoval() {
    paparazzi.snapshot {
      SimpleSettingsScreen(darkMode = false, useUsTimeFormat = true, useDoubleTapForRemoval = true)
    }
  }
}
