 ...
 *
 * This test class demonstrates how to test the app update functionality without requiring the app
 * to be installed from the Play Store.
 */
...
class AppUpdateTest {
  ...
  fun testPrep() {
    runBlocking {
      ...
      composeTestRule
        .onNodeWithTag("about_menu_option", useUnmergedTree = true)
        .assertExists()
        .assertIsEnabled()
        .performClick()
      ...
      composeTestRule.waitUntil(timeoutMillis = 10000) {
        try {
          ...
          val nodes = composeTestRule.onAllNodesWithTag("about_section_card").fetchSemanticsNodes()
          nodes.isNotEmpty()
        } catch (e: Exception) {
          false
        }
      ...
      composeTestRule.waitUntil(timeoutMillis = 10000) {
        try {
          val nodes =
            composeTestRule.onAllNodesWithText("Checking for updates...").fetchSemanticsNodes()
  ...
  /**
   * Tests a flexible update flow where the update completes successfully. This test verifies both
   * the update manager state and the UI interactions.
   */
  @Test
  fun testFlexibleUpdate_Completes() {
    runBlocking {
      ...
      // Wait for navigation to complete and the version section card to be displayed with increased
      // timeout
      composeTestRule.waitUntil(timeoutMillis = 10000) {
        try {
          // Try to find the version_section_card and ensure it's displayed
      ...
      assertEquals(
        TestAppUpdateManager.UpdateStatus.Checking,
      ...
      composeTestRule.waitUntil(timeoutMillis = 5000) {
        composeTestRule
          .onAllNodesWithText(text = "Downloading update:", substring = true)
          .fetchSemanticsNodes()
          .isNotEmpty()
      }
      ...
      assertTrue(
        testAppUpdateManager.updateStatus.first() is TestAppUpdateManager.UpdateStatus.Downloading
      )
      ...
      assertEquals(
        TestAppUpdateManager.UpdateStatus.Downloaded,
        testAppUpdateManager.updateStatus.first()
      )
      ...
      assertEquals(
        TestAppUpdateManager.UpdateStatus.Installing,
        testAppUpdateManager.updateStatus.first()
      )
      ...
      assertEquals(
        TestAppUpdateManager.UpdateStatus.Installed,
        testAppUpdateManager.updateStatus.first()
      )
    }
  }
  ...
  /**
   * Tests an immediate update flow where the update completes successfully. This test verifies both
   * the update manager state and the UI interactions.
   */
  @Test
  ...
  /**
   * Tests a flexible update flow where the download fails. This test verifies both the update
   * manager state and the UI interactions.
   */
  @Test
  fun testFlexibleUpdate_DownloadFails() {
    runBlocking {
      ...
      composeTestRule.waitUntil(timeoutMillis = 5000) {
        composeTestRule
          .onAllNodesWithText(text = "Update check failed:", substring = true)
          .fetchSemanticsNodes()
          .isNotEmpty()
      }
      ...
      assertTrue(
        testAppUpdateManager.updateStatus.first() is TestAppUpdateManager.UpdateStatus.Failed
      )
  ...
  /**
   * Tests a scenario where no update is available. This test verifies both the update manager state
   * and the UI interactions.
   */
  @Test
  fun testNoUpdateAvailable() {
    runBlocking {
      ...
      composeTestRule.waitUntil(timeoutMillis = 5000) {
        composeTestRule
          .onAllNodesWithText("No updates available")
          .fetchSemanticsNodes()
          .isNotEmpty()
      }
      ...
      assertEquals(
        TestAppUpdateManager.UpdateStatus.NotRequired,
        testAppUpdateManager.updateStatus.first()
      )
  ...
  /**
   * Tests a scenario where the user cancels the update. This test verifies both the update manager
   * state and the UI interactions.
   */
  @Test
  fun testUpdateCanceled() {
    runBlocking {
      ...
      testAppUpdateManager.onActivityResult(
        TestAppUpdateManager.UPDATE_REQUEST_CODE,
        Activity.RESULT_CANCELED
      )

      // Verify that update status is "Canceled"
      assertEquals(
        TestAppUpdateManager.UpdateStatus.Canceled,
        testAppUpdateManager.updateStatus.first()
      )
