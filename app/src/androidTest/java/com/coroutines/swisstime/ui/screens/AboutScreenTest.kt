...
class AboutScreenTest {
    ...
    Card(
      ...
      Column(modifier = Modifier.padding(16.dp)) {
        ...
        Column(
          ...
          Button(onClick = {}, modifier = Modifier.fillMaxWidth(0.6f).testTag("rate_app_button")) {
  ...
  private fun TestAppVersionSection() {
    Card(
      modifier = Modifier.fillMaxWidth().testTag("version_section_card"),
      colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface)
    ) {
      Column(modifier = Modifier.padding(16.dp)) {
        Text(
        ...
        Column(
          ...
          Button(
            onClick = {},
            modifier = Modifier.fillMaxWidth(0.6f).testTag("check_updates_button")
          ) {
  ...
  fun testAboutScreenContent() {
    ...
    composeTestRule
      .onNodeWithText(
        "World Timezone Clock is an elegant app that displays the time across different timezones with beautifully crafted mechanical watch faces. Each watch face is designed with attention to detail, mimicking the craftsmanship of real luxury timepieces."
      )
      .assertIsDisplayed()

    // Verify Rate the App section using test tags
    composeTestRule.onNodeWithTag("rate_app_title").assertIsDisplayed()

    // Find the rate app description using the test tag
    composeTestRule.onNodeWithTag("rate_app_description").assertIsDisplayed()

    // Use the test tag to specifically target the rate app button
    composeTestRule.onNodeWithTag("rate_app_button").assertIsDisplayed().assertIsEnabled()

    // Verify App Version section using test tags
    composeTestRule.onNodeWithTag("app_version_title").assertIsDisplayed()

    // Verify version info using test tag
    composeTestRule.onNodeWithTag("app_version_info").assertIsDisplayed()

    // Verify Check for Updates button using test tag
    composeTestRule.onNodeWithTag("check_updates_button").assertIsDisplayed().assertIsEnabled()

    // Test Check for Updates button interaction using test tag
    composeTestRule.onNodeWithTag("check_updates_button").performClick()
  }
  ...
  fun testRateAppButtonInteraction() {
    ...
    composeTestRule.onNodeWithTag("rate_app_button").performClick()
  }
}
