...
class AboutScreenTest : TestCase() {
  ...
  fun testAboutScreenContent() = run {
    ...
    before { composeTestRule.setContent { AboutScreen() } }
    ...
    step("Verify Rate the App section") {
      composeTestRule.onNodeWithText("Rate the App").assertIsDisplayed()
    ...
    step("Verify App Version section") {
      ...
      composeTestRule.onNodeWithText("Check for Updates").assertIsDisplayed().assertIsEnabled()
    }
  ...
  fun testRateAppButtonInteraction() = run {
    ...
    step("Click Rate the App button") {
      composeTestRule.onNodeWithText("Rate the App", useUnmergedTree = true).performClick()
