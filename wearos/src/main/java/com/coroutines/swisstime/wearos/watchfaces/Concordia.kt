...
private object ConcordiaTheme : WorldClockWatchTheme() {
  ...
  private fun drawClockFace(center: Offset, radius: Float): DrawScope.() -> Unit = {
    ...
    drawRect(color = ClockFaceColor, topLeft = Offset(0f, 0f), size = Size(size.width, size.height))
  ...
  private fun drawHourMarkersAndNumbers(center: Offset, radius: Float): DrawScope.() -> Unit = {
    ...
    for (i in 1..12) {
      ...
      rotate(degrees = (i - 3) * 30f, pivot = Offset(markerX, markerY)) {
        drawRect(
          color = MarkersColor,
    ...
    for (i in 0..3) {
      ...
      val numberRadius =
        when (i) {
          0,
          2 -> radius * 0.7f // 12 and 6 o'clock
          1,
          3 -> radius * 0.75f // 3 and 9 o'clock (push out more for rectangular shape)
          else -> radius * 0.7f
        }
      ...
      drawContext.canvas.nativeCanvas.drawText(positions[i].toString(), numberX, numberY, textPaint)
    }
  }
  ...
  if (!isScreenRound) {
    ...
    Box(modifier = modifier.fillMaxSize()) {
      ...
      Canvas(modifier = Modifier.fillMaxSize()) {
        ...
        ConcordiaTheme.staticElementsDrawer.forEach { drawer -> drawer(center, radius)(this) }
      }
      ...
      Box(
        ...
        CustomWorldMapWithDayNight(
          modifier =
            Modifier.fillMaxWidth(0.33f) // Make the map 33% of the width (1/3 smaller than 50%)
      ...
      Canvas(modifier = Modifier.fillMaxSize()) {
        ...
        val hour =
          currentTime.get(Calendar.HOUR) // Use Calendar.HOUR directly for 12-hour format (1-12)
        ...
        println(
          "[DEBUG_LOG] HOUR_OF_DAY: ${currentTime.get(Calendar.HOUR_OF_DAY)}, HOUR: ${currentTime.get(Calendar.HOUR)}, AM_PM: ${if (currentTime.get(Calendar.AM_PM) == Calendar.AM) "AM" else "PM"}"
        )
        ...
        println(
          "[DEBUG_LOG] Current time: ${java.text.SimpleDateFormat("HH:mm:ss").format(currentTime.time)}"
        )
        ...
        // Subtract 90 degrees to align 12 o'clock with vertical top (0 degrees in standard watch
        // convention)
...
fun ConcordiaPreview() {
  Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) { Concordia() }
}
