...
private object LucernaRomaTheme : WorldClockWatchTheme() {
  ...
  fun drawLogo(center: Offset, radius: Float): DrawScope.() -> Unit = {
    ...
    drawContext.canvas.nativeCanvas.drawText("ROMA", center.x, center.y - radius * 0.35f, romaPaint)
  }
}
...
fun LucernaRoma(
  ...
  Box(modifier = modifier.fillMaxSize()) {
    ...
    Canvas(modifier = Modifier.fillMaxSize()) {
      ...
      LucernaRomaTheme.staticElementsDrawer.forEach { drawer -> drawer(center, radius)(this) }
    }
    ...
    Canvas(modifier = Modifier.fillMaxSize()) {
      ...
      rotate(hourAngle) { LucernaRomaTheme.hourHandDrawer(center, radius)(this) }

      // Draw minute hand - using the same logic as the main app
      val minuteAngle = minute * 6f
      println("[DEBUG_LOG] Minute angle: $minuteAngle")
      rotate(minuteAngle) { LucernaRomaTheme.minuteHandDrawer(center, radius)(this) }

      // Draw second hand
      val secondAngle = second * 6f
      rotate(secondAngle) { LucernaRomaTheme.secondHandDrawer(center, radius)(this) }
...
fun LucernaRomaPreview() {
  Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) { LucernaRoma() }
}
