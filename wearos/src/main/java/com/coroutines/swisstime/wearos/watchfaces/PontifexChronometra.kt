...
private object PontifexChronometraTheme : WorldClockWatchTheme() {
  ...
  private fun drawClockFace(center: Offset, radius: Float): DrawScope.() -> Unit = {
    ...
    // Draw the main face
    drawCircle(color = ClockFaceColor, radius = scaledRadius * 0.97f, center = center)
...
fun PontifexChronometra(
  ...
  Box(modifier = modifier.fillMaxSize()) {
    ...
    Canvas(modifier = Modifier.fillMaxSize()) {
      ...
      rotate(hourAngle) { PontifexChronometraTheme.hourHandDrawer(center, radius)(this) }

      // Draw minute hand
      val minuteAngle = minute * 6f
      rotate(minuteAngle) { PontifexChronometraTheme.minuteHandDrawer(center, radius)(this) }

      // Draw second hand
      val secondAngle = second * 6f
      rotate(secondAngle) { PontifexChronometraTheme.secondHandDrawer(center, radius)(this) }
